...
class dotim():
    ...
    def conv_deconv_process_2D(self, i, times, binshape=(2,2), v=0):
        '''
        ...
        for t in range(times): # CONVOLUTION
            ...
            if c == 0:
                ...
                pool_n, maxList_n, _ = self.poolAndMax2D(i, binshape=binshape)
            else:
                pool_n, maxList_n, _ = self.poolAndMax2D(poolList[c-1], binshape=binshape)
            poolList[t] = pool_n
            maxList[t] = maxList_n
            c+=1
        ...
        for t in range(times): # UPCONVOLUTION
            ...
            if c == times:
                deconv_n = self.deconv2D(pooled_mask, maxList[c-1], binshape=binshape) # Deconvolve using the binned mask image and the position of the brightest pixel
            else:
                deconv_n = self.deconv2D(deconv[times-c-1], maxList[c-1], binshape=binshape)
            deconv[t] = deconv_n
            c-=1
    ...
    def conv_deconv(self, times=3, binshape=(2, 2), v=''):
        ...
        if self.sDim == 2 and self.time_dim is False: # CASE 2D AND NO TIME
            output = self.conv_deconv_process_2D(self.im, times, binshape)
            
        if self.sDim == 2 and self.time_dim is True: # CASE 2D AND TIME
            counter = 0
            total = self.im.shape[0]
            for i in self.im: # Do the convolution process for every image
                output.append(self.conv_deconv_process_2D(i, times, binshape))
                counter += 1
        ...
        if self.sDim == 3 and self.time_dim is True and self.treat_3D_as_2D is True: # CASE 3D AND TIME AND TREAT 2D
            ...
            for i in self.im: # Repeat the process as many times as timepoints
                ...
                for j in i: # Repeat the process for every image
                    output_i.append(self.conv_deconv_process_2D(j, times, binshape))
                counter += 1
                print(f"{counter}/{total}", end="\r") if v > 0 else None
